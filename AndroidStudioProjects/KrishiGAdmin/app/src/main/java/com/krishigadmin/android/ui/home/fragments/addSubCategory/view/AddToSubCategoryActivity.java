package com.krishigadmin.android.ui.home.fragments.addSubCategory.view;
...
import com.krishigadmin.android.databinding.ActivityAddToSubCategoryBinding;
...
@AndroidEntryPoint
public class AddToSubCategoryActivity extends BaseActivity<ActivityAddToSubCategoryBinding> {
    SubCategoryModel viewModel;
    String from = "", subCategoryId = "", subCategoryName = "", categoryName = "";
    ...
    String categoryId = "", subImageUrl = "";


    @Override
    protected ActivityAddToSubCategoryBinding getViewBinding() {
        return ActivityAddToSubCategoryBinding.inflate(getLayoutInflater());
    }

    @Override
    protected ActivityNavigator getActivityNavigator() {
        return new ActivityNavigator(AddToSubCategoryActivity.this);
    }
    ...
    protected void initializeObject() {
        ...
        if (getIntent().getExtras() != null) {
            ...
            subCategoryId = getIntent().getExtras().getString(AppConstants.Extras.SUBCATEGORY_ID);
            subCategoryName = getIntent().getExtras().getString(AppConstants.Extras.SUBCATEGORY_NAME);
            categoryName = getIntent().getExtras().getString(AppConstants.Extras.CATEGORY_NAME);

            if (subCategoryName != null) {
                subImageUrl = getIntent().getExtras().getString(AppConstants.Extras.SUBCATEGORY_IMAGE);
            }


        }
        if (subCategoryName != null) {
            viewBinding.subCategoryNameTextInputEditText.setText(subCategoryName);
        }
        if (categoryName != null) {
            viewBinding.selectCategoryAppCompatAutoCompleteTextView.setText(categoryName);
        }
        if (subImageUrl != null) {
    ...
    protected void observeViewModel() {
        ...
        final Observer<ArrayList<Category>> getCategoryUserSuccess = new Observer<ArrayList<Category>>() {
            @Override
            public void onChanged(ArrayList<Category> employees) {
                hideProgressDialog();
                vehicleTypeArraylist = employees;
                vehicleTypeSearchArrayList.clear();
                for (Category employees1 : vehicleTypeArraylist) {
                    vehicleTypeSearchArrayList.add(new Search(employees1.getId(), employees1.getCategory_name()));
                }

                if (categoryName != null) {
                    for (int i = 0; i < vehicleTypeSearchArrayList.size(); i++) {
                        if (vehicleTypeArraylist.get(i).getCategory_name().equalsIgnoreCase(categoryName)) {
                            categoryId = vehicleTypeArraylist.get(i).getId();
                            System.out.println("=====lastCategoryId=====" + categoryId);
        ...
        final Observer<ArrayList<Category>> uploadImageUserSuccess = new Observer<ArrayList<Category>>() {
            ...
            public void onChanged(ArrayList<Category> category) {
                ...
                System.out.println("fasdf" + category.get(0).getUrl());
                subImageUrl = category.get(0).getUrl();
                GlideImageLoader.load(AddToSubCategoryActivity.this,
                        subImageUrl,
                        R.drawable.image_product,
                        R.drawable.image_product,
                        viewBinding.productImageView,
        ...
        final Observer<SubCategory> editSubCategoryUserSuccess = new Observer<SubCategory>() {
            @Override
    ...
    protected void setOnClickListener() {
        ...
        viewBinding.submitButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String category = viewBinding.selectCategoryAppCompatAutoCompleteTextView.getText().toString().trim();
                String subCategory = viewBinding.subCategoryNameTextInputEditText.getText().toString().trim();
                if (category.equalsIgnoreCase("")) {
                    viewBinding.selectCategoryTextInputLayout.setError("Please Select Category");
                    viewBinding.selectCategoryTextInputLayout.setErrorEnabled(true);
                } else if (subCategory.equalsIgnoreCase("")) {
                    viewBinding.subCategoryNameInputLayout.setError("Please Enter Category");
                    viewBinding.subCategoryNameInputLayout.setErrorEnabled(true);
                } else if (subImageUrl.equalsIgnoreCase("")) {
                    Toast.makeText(AddToSubCategoryActivity.this, "Please Upload Sub Category Image", Toast.LENGTH_SHORT).show();
                    showSnackBar(viewBinding.addToSubCategoryActivity, "Please Upload Sub Category Image");
                } else {
                    viewBinding.subCategoryNameInputLayout.setError("");
                    viewBinding.subCategoryNameInputLayout.setErrorEnabled(false);
                    viewBinding.selectCategoryTextInputLayout.setError("");
                    viewBinding.selectCategoryTextInputLayout.setErrorEnabled(false);

                    if (from.equalsIgnoreCase("0")) {
                        try {
                            JSONObject jsonObject = new JSONObject();
                            jsonObject.put("sub_category_name", subCategory);
                            jsonObject.put("category_id", categoryId);
                            jsonObject.put("image_url", subImageUrl);
                            RequestBody requestBody = RequestUtils.createRequestBodyForString(jsonObject.toString());
                            viewModel.subCategory(requestBody, "application/json", "application/json",
                                    sharedPreferencesHelper.getKeyToken());
                            showProgressDialog();
                        ...
                        if (subCategoryId != null) {
                            try {
                                ...
                                jsonObject.put("image_url", subImageUrl);
                                RequestBody requestBody = RequestUtils.createRequestBodyForString(jsonObject.toString());
                                viewModel.editSubCategories(subCategoryId, requestBody, "application/json", "application/json",
                                        sharedPreferencesHelper.getKeyToken());
                                showProgressDialog();
                            } catch (Exception exception) {
        ...
        viewBinding.cameraLinearLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (managePermission.hasPermission(MULTIPLE_PERMISSIONS)) {
                    ...
                    showPictureDialog(AddToSubCategoryActivity.this);
                } else {
        ...
        viewBinding.selectCategoryAppCompatAutoCompleteTextView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (vehicleTypeSearchArrayList.size() != 0) {
                    SearchUtils.searchDialog(AddToSubCategoryActivity.this, "Select Category", vehicleTypeSearchArrayList, new SelectListener() {
                        @Override
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        switch (which) {
                            ...
                            case 1:
                                ...
                                Intent captureImageIntent = createUri(AddToSubCategoryActivity.this, AddToSubCategoryActivity.this, customDirectoryName, fileName, extension);
                    ...
                    if (result.getResultCode() == Activity.RESULT_OK) {
                        ...
                        if (data != null) {
                            ...
                            if (imageUri != null) {
                                ...
                                realPath = RealPathUtils.getRealPath(AddToSubCategoryActivity.this, imageUri);
                                ...
                                File compressFile;
                                try {
                                    compressFile = Compressor.getDefault(AddToSubCategoryActivity.this).compressToFile(uncompressFile);
