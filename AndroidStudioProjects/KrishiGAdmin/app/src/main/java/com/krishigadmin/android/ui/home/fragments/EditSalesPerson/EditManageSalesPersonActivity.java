package com.krishigadmin.android.ui.home.fragments.EditSalesPerson;
...
import com.krishigadmin.android.databinding.ActivityEditManageSalesPersonBinding;
...
@AndroidEntryPoint
public class EditManageSalesPersonActivity extends BaseActivity<ActivityEditManageSalesPersonBinding> {
    ...
    @Override
    protected ActivityEditManageSalesPersonBinding getViewBinding() {
        return ActivityEditManageSalesPersonBinding.inflate(getLayoutInflater());
    }

    @Override
    protected ActivityNavigator getActivityNavigator() {
        return new ActivityNavigator(EditManageSalesPersonActivity.this);
    }
    ...
    protected void initializeObject() {
        if (getIntent().getExtras() != null) {
            userId = getIntent().getExtras().getString(AppConstants.Extras.MANAGE_SALES_ID);
        }
        if (userId != null) {
            viewModel.getUserWithId(userId, "application/json", "application/json",
    ...
    protected void observeViewModel() {
        ...
        viewModel.getUserWithIdError().observe(this, getSubCategoryUserError);

        final Observer<User> getSubCategoryUserSuccess = new Observer<User>() {
            @Override
            public void onChanged(User categories) {
                hideProgressDialog();
                viewBinding.userNameTextInputEditText.setText(categories.getUsername());
                viewBinding.fullNameTextInputEditText.setText(categories.getFullname());
                viewBinding.emailTextInputEditText.setText(categories.getEmail());
                viewBinding.genderAppCompatAutoCompleteTextView.setText(categories.getGender());
                viewBinding.mobileNumberTextInputEditText.setText(categories.getMobileNumber());
                viewBinding.aadhaarTextInputEditText.setText(categories.getAdhaarNumber());
                viewBinding.passwordTextInputEditText.setText(categories.getPassword());
                viewBinding.houseNumberTextInputEditText.setText(categories.getHouseNumber());
                viewBinding.landMarkTextInputEditText.setText(categories.getLandmark());
                viewBinding.pinCodeTextInputEditText.setText(categories.getPincode());
                viewBinding.districtTextInputEditText.setText(categories.getDistrict());
                viewBinding.cityTextInputEditText.setText(categories.getCity());
                viewBinding.stateTextInputEditText.setText(categories.getState());
                ArrayAdapter<String> adapter = new ArrayAdapter<String>(EditManageSalesPersonActivity.this, android.R.layout.simple_dropdown_item_1line, gender);
                viewBinding.genderAppCompatAutoCompleteTextView.setAdapter(adapter);
            }
        };
        viewModel.getUserWithIdSuccess().observe(this, getSubCategoryUserSuccess);

        Observer<String> editUserError = new Observer<String>() {
            @Override
            public void onChanged(@Nullable String error) {
                hideProgressDialog();
                if (error == null) {
        ...
        viewModel.editUserError().observe(this, editUserError);

        final Observer<User> editUserSuccess = new Observer<User>() {
            @Override
            public void onChanged(User categories) {
                hideProgressDialog();
                finish();
        ...
        viewModel.editUserSuccess().observe(this, editUserSuccess);
            ...
            public void onClick(View view) {
                ...
                if (loginRegisterValidation(
                    ...
                    try {
                        ...
                        viewModel.editUser(userId, requestBody1, "application/json", "application/json", sharedPreferencesHelper.getKeyToken());
                        showProgressDialog();
